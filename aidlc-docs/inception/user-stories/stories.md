...
> 탑승 흐름:
> 1. 프론트엔드가 좌석 센서 목업으로 점유 상태 전달 (몇 명, 어느 좌석)
> 2. 운전자가 프로필 인증 (DDB 등록 프로필 중 선택) → driver=true 확정
> 3. 동승자: 등록 프로필(가족) / 미등록 가족 / 게스트 중 하나로 분류

**Acceptance Criteria:**

| # | Given | When | Then |
|---|---|---|---|
| 1 | 운전석 점유 감지 + 운전자가 프로필 인증 완료 | 시동 ON | driver=true 설정 → LTM 조회 → 개인화 환영 (3초 이내) |
| 2 | 동승석 점유 감지 + 등록된 가족 프로필 존재 | 동승자 첫 발화 | 3단계 매핑(채널→자기소개→휴리스틱) → 등록 프로필 매칭 → 맞춤 환영 |
| 3 | 동승석 점유 감지 + 미등록 가족 (DDB에 없음) | 동승자 첫 발화 + 자기소개 | 임시 프로파일 생성 (이름, 추정 age_group) → family 권한 부여 |
| 4 | 동승석 점유 감지 + 매핑 실패 (자기소개 미응답) | 3초 타임아웃 | relationship=guest 자동 처리 + 기본 환영 |
